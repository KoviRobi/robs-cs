...
cons), we create a function which takes one function for each case (each option
in the pattern match) and calls the correct one with the values contained in
that case.

Church encoding is only a little more complicated, because here for recursive
values (e.g. the tail of the list), we call the value contained within with the
arguments for the current case).
]
]
...
The type ```ocaml 't2``` must be the same as a function type ```ocaml 't3 -> 't4```.
...
#let mkhi(hi) = local(display-icon: false, display-name: false, highlights: hi)[
#v(-0.5em)
```ocaml
...
Using the type variable ```ocaml 'el``` to denote the type of ```ocaml el```
for simplicity.
...
The type ```ocaml 't4``` must be the same as a function type ```ocaml 't5 -> 't6```.
...
The type ```ocaml 't6``` must be the same as a function type ```ocaml 't7 -> 't8```.
...
The expression ```ocaml (f el) ((lst f) init)``` has type ```ocaml 't8```. \
It is a function application, so ```ocaml (f el)``` must have type
```ocaml 't9 -> 't8``` and ```ocaml (lst f) init``` have type ```ocaml 't9```
...
= Exercise
Can you work out the type of the Scott encoded ```ocaml cons```? (It is easier
than the Church encoded ```ocaml cons```.)
